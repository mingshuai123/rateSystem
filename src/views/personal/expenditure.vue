<template>
  <div class="app-container subpagePage">
    <div class="rightBox" id="app">
        ...
        <div class="subpageHeader">我的消费</div>
        <div class="expendList">
            <div class="item" v-for="(item,index) in creditsList" :key="index">
                <div class="itemimg"><img :src='"//static.iasac.org.cn/ratingPC/images/credits/"+item.id+".jpg"' alt=""></div>
                <div class="info">
                    <div class="name" v-cloak>{{item.name}}</div>
                    <div class="num">兑换码 : <span>{{item.num}}</span></div>
                    <div class="desc">兑换门店：魔杰电竞深圳南山旗舰店       兑换有效期：2018-09-22</div>
                </div>
                <div class="state" :class="{default:item.state==0}">{{item.state==1?"可兑换":"已兑换"}}</div>
            </div>
        </div>
    </div>
  </div>
